...
**Capture the experience with the same discipline — the goals' twin.** The same documents that seed the goals — and every later chat — carry Sean's descriptive UX/UI writing: how a feature works AND how it feels, in many-paragraph detail. He has never proposed a feature without it, and **that prose IS the functional spec**: he supplies behavior + feel; deriving the backend and data shapes from it is the agent's job. Route it, faithfully and at arrival, into the architecture doc's **Functionality & Experience Specification** section (the canonical behavior repository — `GOALS.md` keeps the distilled *why*; this section keeps the described *how it behaves and feels*, per surface). It must survive every distillation: a doc restructure, a packet assembly, or a design handoff **harvests from it — it never thins it**. And **no surface is ever called "unscoped," "data-free," or "pending" until the corpus has been harvested for his prose about it**: a gap-flag means "described behavior X still lacks endpoint/envelope Y," never "no plan exists." This rule exists because the Claude-Design shift once made UX feel like nobody's job on the CC side — UI/UX was inherently baked in when planners also built their own UI, and the handoff model let his written work fall into the seam and get presented back to him as missing.
