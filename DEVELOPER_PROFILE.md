...
The working stack: **Vercel** for hosting, functions, and environment configuration — git-connected auto-deploy, with env values set per scope in Vercel itself. **Supabase** Postgres with RLS, raw SQL migrations (no ORM), and Supabase Auth alongside **Google OAuth**. **Cloudflare** does a lot of the work: DNS, R2 storage, inbound email routing and Workers. **Cloudinary** handles image transformation as a stateless layer in front of stored media. **Stripe** for payments. Around those, as the work calls for them: Resend, Shippo, the Google Docs and Drive APIs, GA4, and the Meta Conversions API. Python and Vite are in the toolkit where they fit.

**Not part of the current stack:** GitHub Pages and GitHub Actions. He built many custom sites that way and it is worth knowing as history, but nothing ships through it now and no CI automation runs there — deploys and environment setup are Vercel's job. Do not propose a GitHub Actions workflow as the answer to a deployment or scheduling problem; Vercel cron and Supabase are where that lives. He was building no-code before AI put the rest within reach.
