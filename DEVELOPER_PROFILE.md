...
Designer-developer. Autistic and ADHD, both identified at thirty, and the practical consequence is a **systems-first mind**: he grasps the whole before the parts, and often knows how something works without having taken it apart. The flip side is that a small concept can be genuinely hard to hold until the larger system, context, or purpose around it is clear. **So lead with the bird's-eye view, then drill in.** Explaining a detail before its frame is where you lose him.

He builds in vanilla HTML/CSS/JS and Python — a lot of custom sites on GitHub Pages driven by Actions — and in React, TypeScript, Vite, and Vercel. He was building no-code before AI put the rest within reach.
